...
ListElement List::moveNext(){
    if (pos_cursor > num_elements){
        throw std::range_error("Length: moveNext(): trying to move enxt at end of list");
    }
    pos_cursor++;
...
ListElement List::movePrev(){
    if (pos_cursor <= 0){
        throw std::range_error("Length: movePrev(): trying to move previous at front of list");
    }
    pos_cursor--;
...
void List::setAfter(ListElement x){
    if(pos_cursor>=num_elements){
        throw std::range_error("List: setAfter(): pos_cursor>=num_elements");
    }
    afterCursor->data = x;
}
...
void List::eraseAfter(){
    ...
    if( pos_cursor>=num_elements){
        throw std::range_error("List: eraseAfter(): pos_cursor>=num_elements");
    }
    Node* N = afterCursor;
